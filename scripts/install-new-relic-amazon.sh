...
function main () {
  ...
  grep -w amzn $RELEASE_FILE || {
    err "This script is only for Amazon Linux servers."
    exit 1
  }
  ...
  if optional_parameter_exists "--infra" "$@"; then
    ...
    if rpm -qa |grep newrelic-infra; then
      ...
      # Repository
      if grep -q -i 'VERSION_ID="2018' $RELEASE_FILE; then
      ...
      elif grep -q -i 'VERSION_ID="2"' $RELEASE_FILE; then
